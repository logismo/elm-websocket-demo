...
{
	if (region.T.A === region._.A)
	{
		return 'on line ' + region.T.A;
	}
	return 'on lines ' + region.T.A + ' through ' + region._.A;
}
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {ab: fragment, ad: host, ai: path, ak: port_, ao: protocol, ap: query};
	});
...
var $author$project$Main$init = function (flags) {
	return _Utils_Tuple2(
		{F: 'bubble', I: _List_Nil, R: $elm$core$Maybe$Nothing},
		...
		switch (msg.$) {
			...
			case 1:
				...
				return _Utils_Tuple2(
					...
					$elm$core$Platform$Cmd$none);
			case 3:
				var selectedValue = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							F: A2($elm$core$Maybe$withDefault, 'bubble', selectedValue)
						}),
					$elm$core$Platform$Cmd$none);
			default:
...
var $author$project$Main$Send = {$: 0};
...
var $abadi199$elm_input_extra$Dropdown$dropdown = F3(
	function (options, attributes, currentValue) {
		var itemsWithEmptyItems = function () {
			...
			if (!_v1.$) {
				var emptyItem = _v1.a;
				return A2($elm$core$List$cons, emptyItem, options.aP);
			} else {
				return options.aP;
			}
		}();
		...
		return A2(
			...
			_Utils_ap(
				...
				_List_fromArray(
					[
						A2($abadi199$elm_input_extra$Dropdown$onChange, options.G, options.P)
					])),
			A2($elm$core$List$map, toOption, itemsWithEmptyItems));
	});
var $author$project$Main$DropdownChanged = function (a) {
	return {$: 3, a: a};
};
var $abadi199$elm_input_extra$Dropdown$defaultOptions = function (onChangeHandler) {
	return {G: $elm$core$Maybe$Nothing, aP: _List_Nil, P: onChangeHandler};
};
var $author$project$Main$dropdownOptions = function () {
	var defaultOptions = $abadi199$elm_input_extra$Dropdown$defaultOptions($author$project$Main$DropdownChanged);
	return _Utils_update(
		defaultOptions,
		{
			aP: _List_fromArray(
				[
					{K: true, U: 'Bubble Sort', W: 'bubble'},
					{K: true, U: 'Selection Sort', W: 'selection'},
					{K: true, U: 'Merge Sort', W: 'merge'}
				])
		});
}();
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$text('Elm Websocket Demo')
					])),
				A2(
				$elm$html$Html$p,
				_List_Nil,
				_List_fromArray(
					[
						$elm$html$Html$text('Generates a random array and sorts it using algorithm of your choice through a Python Websocket server')
					])),
				A2(
				$elm$html$Html$h4,
				_List_Nil,
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('Sorting Algorithm: '),
								A3($abadi199$elm_input_extra$Dropdown$dropdown, $author$project$Main$dropdownOptions, _List_Nil, model.R)
							])),
						A2(
...
var $author$project$Main$main = $elm$browser$Browser$element(
	{aO: $author$project$Main$init, a$: $author$project$Main$subscriptions, a1: $author$project$Main$update, a2: $author$project$Main$view});
